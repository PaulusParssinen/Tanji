...
public sealed class IdentifiersGenerator : IIncrementalGenerator
{
    ...
    {
        return string.IsNullOrWhiteSpace(message.Name) || message.Name.StartsWith("_-", StringComparison.Ordinal);
    }
    private static string Generate(MessageItem[] messages, bool isOutgoing)
    {
        ...
        foreach (MessageItem message in messages)
        {
            ...
            string backingFieldVariableName = $"_{char.ToLowerInvariant(message.Name[0])}{message.Name.Substring(1)}";
            indentedText.WriteLine($$"""
